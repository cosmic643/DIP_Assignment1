'''
This program scales down an image using Replication
scaling factor: 2
...
for k in range(colors):
    for i in range(int(height/2)):
        for j in range(int(width/2)):
            scaled_down_img[i][j][k] = img[2*i][2*j][k]
...
def mouse_crop(event, x, y, flags, param):
    ...
    # if the left mouse button was released
        ...
        if len(cropped_coordinates) == 2:  # when two points were found
            roi = scaled_down_img[int(cropped_coordinates[0][1]/2):int(cropped_coordinates[1][1]/2), int(cropped_coordinates[0][0]/2):int(cropped_coordinates[1][0]/2)]
